...
- `shop/` 제거는 비교적 단순하지만, 로그인 후 장바구니 처리와 마이페이지 연결 같은 결합 지점은 먼저 분리해야 한다.
...
아래는 회원 서비스의 성격에 따라 유지 여부를 결정한다.
...
현재 방향이 "계정 시스템만 남긴다"에 가깝다면 조건부 기능도 제거하는 편이 낫다.
현재 방향이 "회원 등급/혜택/운영 기능까지 남긴다"에 가깝다면 포인트와 일부 관리자 화면은 유지할 가치가 있다.
...
위 항목들은 "회원 기능"처럼 보일 수 있지만, 실제 제품 가치보다 유지 비용이 더 크다.
계정 중심 제품으로 재편한다면 1차 범위에서는 빼는 편이 안전하다.
...
위 3개 묶음은 제품 요구사항이 확정된 후 최종 제거 여부를 결정한다.
...
- 유지: 계정, 인증, 회원관리
- 제거: 게시판, 쇼핑몰
- 보류: 포인트, 쪽지, 공개 프로필, 방문통계

보류 항목까지 제거하기로 확정되면 회원 전용 제품으로의 축소 작업이 훨씬 단순해진다.
