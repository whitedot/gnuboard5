...
<aside id="sbsk" class="sbsk">
    <h2>장바구니 <span class="cart-count"><?php echo get_boxcart_datas_count(); ?></span></h2>
    ...
    foreach($cart_datas as $row)
    {
        ...
        echo '</span>'.PHP_EOL;
		echo '</div>';
		echo '<button class="cart_del" type="button" data-it_id="'.$row['it_id'].'"><i aria-hidden="true"></i><span>삭제</span></button>'.PHP_EOL;
        echo '</li>';
    ...
    if ($i==0)
        echo '<li>장바구니 상품 없음</li>'.PHP_EOL;
    ?>
    </ul>
    <?php if($i){ ?><div><button type="submit">구매하기</button></div><?php } ?>
    <a href="<?php echo G5_SHOP_URL; ?>/cart.php">전체보기</a>
    </form>
</aside>
<script>
jQuery(function ($) {
