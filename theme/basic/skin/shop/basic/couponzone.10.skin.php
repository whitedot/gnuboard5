...
<section>
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $coupon .= '<li>'.PHP_EOL;
		$coupon .= '<div>'.PHP_EOL;
        $coupon .= '<div><img src="'.str_replace(G5_PATH, G5_URL, $img_file).'" alt="'.$subj.'">'.PHP_EOL;
        $coupon .= '<div><strong>'.$subj.'</strong><br><span>'.$print_cp_price.'</span></div>'.PHP_EOL;
		$coupon .= '</div>'.PHP_EOL;
		$coupon .= '<div>'.PHP_EOL;
        $coupon .= '<div>'.PHP_EOL;
        $coupon .= '<span>적용</span><button class="coupon_info_btn">'.$cp_target.' <i aria-hidden="true"></i></button>'.PHP_EOL;
        ...
        if( $row['cp_minimum'] ){   // 쿠폰에 최소주문금액이 있다면
        	$coupon .= '<li>최소주문금액 : <span><b>'.number_format($row['cp_minimum']).'</b>원</span></li>';
        }

        $coupon .='</ul>
        <button class="coupon_info_cls"><i aria-hidden="true"></i><span>닫기</span></button>
        </div>'.PHP_EOL;
        $coupon .= '</div>'.PHP_EOL;
        $coupon .= '<div><span>기한</span>다운로드 후 '.number_format($row['cz_period']).'일</div>'.PHP_EOL;
        //cp_1 카테고리할인
        //cp_2 개별상품할인
        //cp_3 주문금액할인
        //cp_4 배송비할인
		$coupon .= '</div>'.PHP_EOL;
        $coupon .= '</div>'.PHP_EOL;
        $coupon .= '<div><button type="button" class="coupon_download" data-cid="'.$row['cz_id'].'">쿠폰다운로드</button></div>'.PHP_EOL;
        $coupon .= '</li>'.PHP_EOL;
    }
...
<section id="point_coupon">
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$coupon .= '<div><span>기한</span>다운로드 후 '.number_format($row['cz_period']).'일</div>'.PHP_EOL;
		$coupon .= '<div><button type="button" class="coupon_download" data-cid="'.$row['cz_id'].'">포인트 '.number_format($row['cz_point']).'점 차감</button></div>'.PHP_EOL;
