<?php
if (!defined('_GNUBOARD_')) exit; // 개별 페이지 접근 불가

// add_stylesheet('css 구문', 출력순서); 숫자가 작을 수록 먼저 출력됨

?>

<!-- 쪽지 보내기 시작 { -->
<div id="memo_write">
    <h1 id="win_title"><i aria-hidden="true"></i> 쪽지 보내기</h1>
    <div>
        <ul>
            <li><a href="./memo.php?kind=recv">받은쪽지</a></li>
            <li><a href="./memo.php?kind=send">보낸쪽지</a></li>
            <li class="selected"><a href="./memo_form.php">쪽지쓰기</a></li>
        </ul>

        <form name="fmemoform" action="<?php echo $memo_action_url; ?>" onsubmit="return fmemoform_submit(this);" method="post" autocomplete="off">
        <div>
            <h2>쪽지쓰기</h2>
            <ul>
                <li>
                    <label for="me_recv_mb_id">받는 회원아이디<strong>필수</strong></label>
                    
                    <input type="text" name="me_recv_mb_id" value="<?php echo $me_recv_mb_id; ?>" id="me_recv_mb_id" required size="47" placeholder="받는 회원아이디">
                    <span>여러 회원에게 보낼때는 컴마(,)로 구분하세요.</span>
                    <?php if ($config['cf_memo_send_point']) { ?>
                    <br ><span>쪽지 보낼때 회원당 <?php echo number_format($config['cf_memo_send_point']); ?>점의 포인트를 차감합니다.</span>
                    <?php } ?>
                </li>
                <li>
                    <label for="me_memo">내용</label>
                    <textarea name="me_memo" id="me_memo" required><?php echo $content ?></textarea>
                </li>
                <li>
                    <span>자동등록방지</span>
                    
                    <?php echo captcha_html(); ?>
                    
                </li>
            </ul>
        </div>

        <div>
        	<button type="submit" id="btn_submit">보내기</button>
        	<button type="button" onclick="window.close();">창닫기</button>
        </div>
    </div>
    </form>
</div>

<script>
function fmemoform_submit(f)
{
    <?php echo chk_captcha_js();  ?>

    return true;
}
</script>
<!-- } 쪽지 보내기 끝 -->