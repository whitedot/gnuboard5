...
function member_count_by_email($email)
{
    return (int) sql_fetch_value_prepared(
        " select count(*) as cnt from {$GLOBALS['g5']['member_table']} where mb_email = :mb_email ",
...
function member_find_email_certify_candidate($mb_id)
{
    return sql_fetch_prepared(
        " select mb_id, mb_email_certify2, mb_leave_date, mb_intercept_date from {$GLOBALS['g5']['member_table']} where mb_id = :mb_id ",
        array('mb_id' => $mb_id)
    );
}
...
function member_clear_email_certify_token($mb_id)
{
    return sql_query_prepared(
        " update {$GLOBALS['g5']['member_table']} set mb_email_certify2 = '' where mb_id = :mb_id ",
        array('mb_id' => $mb_id)
    );
}
...
{
    return sql_query_prepared(
        " update {$GLOBALS['g5']['member_table']} set mb_email_certify = :mb_email_certify where mb_id = :mb_id ",
        array(
...
{
    return sql_fetch_prepared(
        " select mb_email, mb_datetime, mb_ip, mb_email_certify, mb_id from {$GLOBALS['g5']['member_table']} where mb_id = :mb_id ",
        array('mb_id' => $mb_id)
    );
}
...
{
    return sql_fetch_prepared(
        " select mb_name from {$GLOBALS['g5']['member_table']} where mb_id = :mb_id and substring(mb_email_certify, 1, 1) = '0' ",
        array('mb_id' => $mb_id)
    );
}
...
{
    return (int) sql_fetch_value_prepared(
        " select count(*) as cnt from {$GLOBALS['g5']['member_table']} where mb_id <> :mb_id and mb_email = :mb_email ",
        array(
            'mb_id' => $mb_id,
            'mb_email' => $mb_email,
        )
...
{
    return sql_query_prepared(
        " update {$GLOBALS['g5']['member_table']} set mb_email = :mb_email where mb_id = :mb_id ",
        array(
            'mb_email' => $mb_email,
            'mb_id' => $mb_id,
        )
