...
<ul id="theme_list">
    ...
    for($i=0; $i<$total_count; $i++) {
        ...
        if($config['cf_theme'] == $theme[$i]) {
            $btn_active = '<span>사용중</span><button type="button" data-theme="'.$theme[$i].'" '.'data-name="'.$name.'">사용안함</button>';
        } else {
            ...
            $btn_active = '<button type="button" data-theme="'.$theme[$i].'" '.'data-name="'.$name.'" data-set_default_skin="'.$set_default_skin.'">테마적용</button>';
        }
    ?>
    <li>
        <div>
