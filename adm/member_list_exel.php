...
$member_export_view = admin_build_member_export_page_view($_GET, $config, $member_export_runtime);
$filter_state = $member_export_view['filter_state'];
$member_export_links = $member_export_view['links'];
$member_export_client_config = $member_export_view['client_config'];
...
<section class="card mb-5">
    ...
    <div class="card-body space-y-3 text-sm leading-6 text-default-700">
        ...
        <p>
            ...
            <a href="<?php echo $member_export_links['file_delete_url']; ?>" class="font-semibold text-primary hover:underline">회원관리파일 일괄삭제</a>
            를 이용해 주세요.
        </p>
        <p>회원 정보 수정은 <a href="<?php echo $member_export_links['member_list_url']; ?>" class="font-semibold text-primary hover:underline">회원 관리</a>에서 진행하실 수 있습니다.</p>
    </div>
</section>
...
function startExcelDownload() {
    ...
    eventSource = new EventSource(`${memberExportClientConfig.stream_url}?${query}`);
...
function showDownloadPopup() {
    ...
    PopupManager.render(memberExportClientConfig.popup_progress_title, bodyHTML, '', { disableOutsideClose: true });
...
function handlePopupCloseWithConfirm(e) {
    if (eventSource) {
        const confirmClose = confirm(memberExportClientConfig.close_confirm_message);
        if (!confirmClose) {
            e.preventDefault();
            return;
        }
        eventSource.close();
        eventSource = null;
        alert(memberExportClientConfig.download_stopped_message);
    }
...
function handleProgressUpdate() {
    return function(e) {
        ...
        if (status === "progress") 
        {
            ...
            const sec = Math.max(
                memberExportClientConfig.estimated_seconds_min,
                Math.ceil(total * memberExportClientConfig.estimated_seconds_multiplier)
            );
        ...
        else if (status === "done") 
        {
            ...
            titleEl.textContent = memberExportClientConfig.popup_done_title;
...
function handleDownloadError() {
    return function(e){
        ...
        document.querySelector('.progress-summary').innerHTML = memberExportClientConfig.download_failed_summary;
        document.querySelector('.progress-message').innerHTML = memberExportClientConfig.download_failed_message;
