...
if (isset($_POST['email'])) {
    ...
    for ($i=0; $i<count($email); $i++){
        ...
        $real_email[] = $email[$i];
        mailer($config['cf_admin_email_name'], $config['cf_admin_email'], trim($email[$i]), '[메일검사] 제목', '<span>[메일검사] 내용<p>이 내용이 제대로 보인다면 보내는 메일 서버에는 이상이 없는것입니다.<p>'.G5_TIME_YMDHIS.'<p>이 메일 주소로는 회신되지 않습니다.</span>', 1);
    }
...
<section>
    ...
    <fieldset id="fsendmailtest">
        ...
        <label for="email">받는 메일주소<strong> 필수</strong></label>
        <input type="text" name="email" value="<?php echo $member['mb_email'] ?>" id="email" required class="required" size="80">
