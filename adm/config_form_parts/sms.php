    <section id="anc_cf_sms">
        ...
        <div>
            <div>
                                    <div>
                                            <div><label for="cf_sms_use">SMS 사용</label>                    </div>
                                            <div>
                            <select id="cf_sms_use" name="cf_sms_use">
                                ...
                                <option value="icode" <?php echo get_selected($config['cf_sms_use'], 'icode'); ?>>아이코드</option>
                                    ...
                                    <div>
                                            <div><label for="cf_sms_type">SMS 전송유형</label>                    </div>
                                            <div>
                            ...
                            <select id="cf_sms_type" name="cf_sms_type">
                                ...
                                <option value="LMS" <?php echo get_selected($config['cf_sms_type'], 'LMS'); ?>>LMS</option>
                                    ...
                                    <div>
                                            <div><label for="cf_icode_id">아이코드 회원아이디<br>(구버전)</label>                    </div>
                                            <div>
                            ...
                            <input type="text" name="cf_icode_id" value="<?php echo get_sanitize_input($config['cf_icode_id']); ?>" id="cf_icode_id" size="20">
                                    ...
                                    <div>
                                            <div><label for="cf_icode_pw">아이코드 비밀번호<br>(구버전)</label>                    </div>
                                            <div>
                    ...
                    <?php if ($userinfo['payment'] == 'A') { ?>
                                        <div>
                                                <div>충전 잔액<br>(구버전)                    </div>
                                                <div>
                                <?php echo number_format($userinfo['coin']); ?> 원.
                                <a href="http://www.icodekorea.com/smsbiz/credit_card_amt.php?icode_id=<?php echo get_text($config['cf_icode_id']); ?>&amp;icode_passwd=<?php echo get_text($config['cf_icode_pw']); ?>" target="_blank">충전하기</a>
                                                </div>
                                        </div>
                    <?php } ?>
                                    <div>
                                            <div><label for="cf_icode_token_key">아이코드 토큰키<br>(JSON버전)</label>                    </div>
                                            <div>
                            ...
                            <input type="text" name="cf_icode_token_key" value="<?php echo isset($config['cf_icode_token_key']) ? get_sanitize_input($config['cf_icode_token_key']) : ''; ?>" id="cf_icode_token_key" size="40">
